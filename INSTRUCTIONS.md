...
This scraper extracts job listings from Endava SmartRecruiters API (Romania only) and imports them to peviitor.ro.

Target: SmartRecruiters API
...
1. **Start with brand** - We know the brand (e.g., "ENDAVA")
...
8. **Scrape new jobs** - Extract jobs from SmartRecruiters API (Romania)
...
> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts Endava jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from SmartRecruiters API (Romania only)
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape SmartRecruiters API (jobs for Romania)
    │
    ▼
...
| `tests/validate-endava-jobs.js` | CI fast validator (HEAD only); thin CLI over `src/job-validator.js` + `solr.js` |
...
| `tests/e2e/scraper.test.js` | End-to-end tests with real SmartRecruiters API |
