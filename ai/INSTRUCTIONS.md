...
This scraper extracts job listings from ENDAVA Careers Romania API and imports them to peviitor.ro.

Target: https://jobs.smartrecruiters.com/Endava
...
1. **Start with brand** - We know the brand ("ENDAVA")
...
8. **Scrape new jobs** - Extract jobs from ENDAVA Careers API (Romania)
...
> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts ENDAVA Careers jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from ENDAVA Careers API (Romania only)
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape ENDAVA API (jobs for Romania)
    │
    ▼
