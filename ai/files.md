...
| `scraper/job-validator.js` | Shared validation primitives - exports validateByHead(url), validateByContent(url, opts), validateByBrowser(url, opts), DEFAULT_EXPIRED_KEYWORDS. Used by `validate-jobs.js`, `tests/validate-endava-jobs.js`, and the deep-validate workflow. |
...
| `tests/validate-endava-jobs.js` | **ENDAVA-specific validator (used by CI).** Modes: `--head` (default), `--content`, `--browser` (Playwright). Called nightly by `automation-testing.yml` and manually via `job-deep-validate.yml`. Supports `--dry-run` and `--delete`. |
...
| `tests/e2e/scraper.test.js` | E2E tests - full pipeline with real ENDAVA career API, ANAF, and Peviitor API |
...
| `ai/ROBOTS.md` | robots.txt analysis and scraping policy for SmartRecruiters (Endava) |
...
| `SECURITY.md` | Security policy and vulnerability reporting (repo root) |
...
| `.github/workflows/automation-template-sync-check.yml` | Weekly check that this derived scraper is up to date with the EPAM template |
...
| `company.json` (root) | **Gitignored cache** (`/company.json` in `.gitignore`). Refreshed every 7 days. If ANAF is unreachable AND cache is stale, falls back to stale cache rather than failing. |
...
- Full workflow: validate company (ANAF+CUIScan+CUIFirma+Peviitor) → scrape ENDAVA → transform → upsert → generate docs/jobs.md
