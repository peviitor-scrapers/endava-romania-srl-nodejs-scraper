...
**Audience:** AI agents (Claude, etc.) tasked with deriving a new Node.js scraper from this ENDAVA template for a different Romanian company.
...
> **AI agents: after applying this guide to a derived repo, DELETE this file from the derived repo. It belongs only in the ENDAVA template.**
...
4. **ENDAVA template is in `is_template: true` state** (check via `gh api repos/sebiboga/endava-romania-srl-nodejs-scraper -q .is_template`)
...
gh repo create sebiboga/<slug>-nodejs-scraper \
  --template sebiboga/endava-romania-srl-nodejs-scraper \
  --public \
...
# expect: sebiboga/endava-romania-srl-nodejs-scraper
```
...
The badge "Generated from ENDAVA" can only appear if the repo is created via the template feature. If the existing repo was created manually (copy-paste, fork), you must delete and recreate.
...
|---------|---------|----------|
| Paginated JSON API | ENDAVA | GET + loop pages until empty |
...
```bash
rm -f company.json           # template's ANAF cache (ENDAVA identity)
rm -f docs/jobs.md           # template's last scraped ENDAVA jobs list (71+ entries)
```

**Why this matters (Pitfall #10):**
- `company.json` (root): the caching logic in `company.js` will read it first and skip ANAF for the derived company → first scrape uses ENDAVA identity.
- `docs/jobs.md`: contains the template company's last scraped jobs. Bulk sed will rewrite `jobs.smartrecruiters.com/Endava → <your domain>` in URLs, producing fake but plausible-looking URLs (e.g. `https://www.co-era.com/en/vacancy/senior-sap-fico-consultant-blt4lismih6xo9i26pk_en` — domain correct, path is an ENDAVA-only vacancy ID). The GitHub Pages dashboard serves this deceptive content until the first scrape regenerates `docs/jobs.md`. The user sees "71 jobs" while SOLR has 2, and assumes the scraper is broken.
...
Replace the `parseApiJobs` block (ENDAVA-specific) with tests for your new parser:
...
Replace ENDAVA's API fetch with your new scraping method. Increase `beforeAll` timeout to **60s** if your target site is in Romania (Azure GH runners are slow to RO IPs).
...
### 5.7 `tests/validate-endava-jobs.js` — rename

```bash
git mv tests/validate-endava-jobs.js tests/validate-<brand>-jobs.js
```
...
Change `"name": "endava-scraper-tests"` to `"name": "<slug>-scraper-tests"`.

---

## 6. Documentation sweep (high-risk — see pitfalls section)

You will likely use `sed` for a bulk rename of "ENDAVA" → new brand. **This is dangerous.** Read the entire "Pitfalls — bulk sed" section below before running sed.

After sed, **always** do these manual review passes:

1. **README.md** — restore the "Derivat din ENDAVA template" link (sed will have changed it to point at the new repo, which is self-referential)
2. **CONTRIBUTING.md** — replace the inherited "Deriving a New Scraper" checklist with a slim "this is a derived scraper" intro pointing back to ENDAVA template
3. **AGENTS.md** — change "📐 This Repo Is a Template" to "🌱 This Repo Is a Derived Scraper"
4. **ROBOTS.md** — analyze the new target site's `robots.txt` (e.g. Continental Hotels has none → 302 redirect). Keep a "Diferență față de ENDAVA template" section that compares against the ENDAVA constraints.
5. **CHANGELOG.md** — REPLACE with a fresh `1.0.0` entry. Don't keep ENDAVA's version history (it doesn't belong here).
6. **package.json** — set `name` to `scraper-<brand>-ro` and `version` to `1.0.0`.
7. **docs/index.html** — i18n strings still contain "ENDAVA Careers API" / "ENDAVA Romania". Replace with new brand. Make sure the page title fallback constants match the new company.
...
```bash
grep -rn "ENDAVA SYSTEMS\|ENDAVA Careers" .github/workflows/
# expect: no matches
```
...
**No secrets needed.** The ENDAVA template uses the Peviitor API (public, no auth). Derived scrapers follow the same pattern — no `SOLR_AUTH` or other secrets required.
...
Derived from sebiboga/endava-romania-srl-nodejs-scraper."
git push
...
## 11. Update ENDAVA template's "Derived Scrapers" table

After CI is green, add the new repo to ENDAVA's README:
...
If you do this:
```bash
sed -i 's/endava.com/jobs-newcompany.ro/g' file.js
```
...
This is a known schema drift bug — track upstream as issue in ENDAVA template if not already fixed.
...
The template ships with `company.json` (root) containing ENDAVA's ANAF data. The caching logic in `company.js` will read this cache first and skip ANAF for the derived company → first scrape uses ENDAVA identity. **Always `rm -f company.json` early in derivation.**
...
### Pitfall #9 — Forgot to update the ENDAVA template's "Derived Scrapers" list (issue #1 Continental)

This is the last manual step and easy to miss. Set a reminder.

### Pitfall #10 — Stale `docs/jobs.md` from template gets sed-mangled into fake URLs (issue #39 ENDAVA)

The template's `docs/jobs.md` contains the template company's last scraped jobs (e.g. 71 ENDAVA jobs). Bulk sed rewrites brand strings but keeps vacancy paths/IDs, producing URLs like:
- `https://www.co-era.com/en/vacancy/senior-sap-fico-consultant-blt4lismih6xo9i26pk_en` ← domain correct, path is an ENDAVA-only vacancy ID
- Company info table shows ENDAVA's HQ address (`IANCU DE HUNEDOARA, 48, Bucureşti`) instead of the derived company's
...
When the fix is template-wide (would benefit ALL derived scrapers), file in ENDAVA. When it's specific to the derived scraper (like company-specific scraping bugs), file there.
...
|------|--------|-----|---------|--------|
| [endava-romania-srl-nodejs-scraper](https://github.com/sebiboga/endava-romania-srl-nodejs-scraper) | JSON API | 9533457 | Template (this repo) | — |
...
| [ntt-data-romania-sa-nodejs-scraper](https://github.com/sebiboga/ntt-data-romania-sa-nodejs-scraper) | HTML/cheerio (Taleo) | 13091574 | Oracle Taleo multi-page, first SA (not SRL) after Continental | ✅ Live |
...
**ENDAVA template:**
- [#34](https://github.com/sebiboga/endava-romania-srl-nodejs-scraper/issues/34) — Extract config into a single file (the "single source of truth" principle)
- [#35](https://github.com/sebiboga/endava-romania-srl-nodejs-scraper/issues/35) — Shared validator module (`src/job-validator.js`)
- [#36](https://github.com/sebiboga/endava-romania-srl-nodejs-scraper/issues/36) — Derive a second scraper (validation)
- [#37](https://github.com/sebiboga/endava-romania-srl-nodejs-scraper/issues/37) — Sync with remote on PR runs
- [#38](https://github.com/sebiboga/endava-romania-srl-nodejs-scraper/issues/38) — Sync ordering before npm install
