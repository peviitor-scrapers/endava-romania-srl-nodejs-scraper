...
## Project
ENDAVA scraper for peviitor.ro (Node.js, ESM, Jest)

## 🌱 This Repo Is a Derived Scraper
This scraper is derived from the **EPAM template** ([epam-systems-international-srl-nodejs-scraper](https://github.com/sebiboga/epam-systems-international-srl-nodejs-scraper)), the reference implementation for all Node.js scrapers in the peviitor.ro ecosystem.
...
- **Only the API parsing logic in `scraper/index.js`** (`fetchJobsPage`, `parseApiJobs`) is ENDAVA-specific. The output shape (`mapToJobModel`, `transformJobsForSOLR`) must stay uniform across derived scrapers.
...
```bash
gh run view <RUN_ID> --repo peviitor-scrapers/endava-romania-srl-nodejs-scraper --json status -q .status
```
...
**On every session:**
1. Check open GitHub issues: `gh issue list --repo peviitor-scrapers/endava-romania-srl-nodejs-scraper --state open`
