...
const JOB_BASE = "https://jobs.smartrecruiters.com/Endava";
const PAGE_SIZE = 100;
...
async function fetchPage(offset) {
  const url = `${companyConfig.apiBase}&limit=${PAGE_SIZE}&offset=${offset}`;
...
function parseApiJob(job) {
  const remote = job.location?.remote || false;
  const hybrid = job.location?.hybrid || false;
  ...
  const tags = (job.department?.label ? [job.department.label.toLowerCase()] : [])
    .concat(job.function?.label ? [job.function.label.toLowerCase()] : []);

  const postingId = job.ref?.split('/').pop() || job.refNumber;
  const url = `${JOB_BASE}/${postingId}`;

  return { url, title: job.name, workmode, location, tags };
}

function parseApiJobs(apiData) {
  const jobs = apiData.content || [];
  const total = apiData.totalFound || 0;
  return {
    jobs: jobs.map(parseApiJob),
    total
  };
}

async function scrapeAllListings() {
  console.log("Fetching jobs from SmartRecruiters API...");
  const first = await fetchPage(0);
  const total = first.totalFound || 0;
  const allJobs = [...first.content];
  console.log(`Total jobs on SmartRecruiters: ${total}`);
  for (let offset = PAGE_SIZE; offset < total; offset += PAGE_SIZE) {
    const page = await fetchPage(offset);
    allJobs.push(...page.content);
    console.log(`  Fetched ${offset + page.content.length}/${total}`);
  }
  console.log(`Total jobs fetched: ${allJobs.length}`);
  return allJobs.map(parseApiJob);
}
...
async function main() {
  try {
    ...
    console.log(`Jobs scraped from SmartRecruiters: ${scrapedCount}`);
...
export { parseApiJob, parseApiJobs, mapToJobModel, transformJobsForSOLR };
