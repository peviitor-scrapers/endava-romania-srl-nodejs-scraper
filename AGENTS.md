...
- **Only the API parsing logic in `index.js`** (`fetchJobsPage`, `parseApiJobs`) is Endava-specific. The output shape (`mapToJobModel`, `transformJobsForSOLR`) must stay uniform across derived scrapers.
...
# E2E tests (real Endava API, SOLR conditional)
npm run test:e2e
...
- `tests/validate-endava-jobs.js` — CI fast validator (HEAD only); thin wrapper over src/job-validator.js + solr.js
