...
### Added
- Full alignment with EPAM template structure: `scraper/` directory (`index`, `api`, `anaf`, `company`, `demoanaf`, `job-validator`, `markdown-generator`, `validate-jobs`) + `scraper/config/`
- `scraper/api.js` — Peviitor API module replacing direct `solr.js` access (no `SOLR_AUTH` needed)
- `ai/` documentation set (AGENTS, AI-DERIVATION-GUIDE, BRANCH, company-model, files, INSTRUCTIONS, ISSUES, job-model, MAINTENANCE, PUBLIC, ROBOTS, TOPICS, UPDATE-REPO-ABOUT, VERIFY)
- Workflows `automation-template-sync-check.yml` + `job-deep-validate.yml` (now 5 total)

### Changed
- `scraper/index.js`: stale-job deletion, `--test` flag, 0-jobs upload guard, jobs written to `scraper/jobs.json`, `docs/company.json` synced from config
- `scraper/config/company.json` + `docs/company.json` → template schema (`id`, `company`, `brand` UPPERCASE `ENDAVA`, `status`, `location[]`, `website[]`, `career[]`, `scraperFile`)
- Tests: imports moved to `../../scraper/*`; SOLR verification via `api.getCompanyByCif`/`api.querySOLR`; `itIfApi`/`itIfAnaf` gating; `solr.test.js` removed
- README.md aligned to EPAM template structure
...
- `tests/validate-endava-jobs.js`: multi-mode validator (`--head`, `--content`, `--browser`, `--timeout`)
...
### Changed
- `validate-jobs.js` and `tests/validate-endava-jobs.js` are now thin CLIs that delegate to `src/job-validator.js` and `solr.js` (closes #35)
- `tests/validate-endava-jobs.js` no longer reimplements SOLR auth, query, or delete — uses `querySOLR` + `deleteJobByUrl` from `solr.js`
...
### Changed
- `index.js`, `company.js`, `demoanaf.js`, `tests/validate-endava-jobs.js`, `docs/index.html`, `automation-testing.yml` all now read from `config/company.json` instead of hardcoded constants
...
### Added
- Initial release
- Job scraping from ENDAVA Careers Romania API
